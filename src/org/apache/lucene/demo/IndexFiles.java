...
public class IndexFiles {
   ...
   * @param indexSavePath 索引文件保存路径
   */
  public void indexFiles(String search_dir,String indexSavePath){
    File indexDir = null;
    if(indexSavePath==null)
    	indexDir = INDEX_DIR; 
    else
	  indexDir = new File(indexSavePath); 
	final File docDir = new File(search_dir);
	if (!docDir.exists() || !docDir.canRead()) {
	  System.out.println("Document directory '" +docDir.getAbsolutePath()+ "' does not exist or is not readable, please check the path");
	  return;
	}
